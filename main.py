# main.py - Backend Optimizado para Bot de Trading Opciones Binarias Pro
...
try:
    ...
    IQ_AVAILABLE = False
    # Para desarrollo/testing cuando IQOptionAPI no está disponible
    logger.warning("⚠️ Ejecutando en modo simulación sin IQOptionAPI")
...
def calculate_binary_indicators(candles, strategy: Strategy = None):
    ...
    try:
        ...
        return {
            ...
            "rsi_oversold": calculate_rsi(closes) <= 30,
            "rsi_overbought": calculate_rsi(closes) >= 70,
            ...
            "bb_squeeze": round((bb_upper - bb_lower) / sma20 * 100, 2),
...
class BinaryOptionsStrategies:
    ...
    def macd_signal_strategy(indicators):
        ...
        # SEÑAL CALL: MACD bullish + confirmaciones
        if indicators['macd_bullish']:
            signals.append("call")
            confidence += 35
            
            if indicators['rsi'] > 45 and indicators['rsi'] < 70:
                confidence += 15
            if indicators['ema_bullish_alignment']:
                confidence += 20
        
        # SEÑAL PUT: MACD bearish + confirmaciones
        if not indicators['macd_bullish']:
            signals.append("put")
            confidence += 35
            
            if indicators['rsi'] < 55 and indicators['rsi'] > 30:
                confidence += 15
    ...
    def triple_ema_strategy(indicators):
        """Estrategia Triple EMA agresiva"""
        signals = []
        confidence = 0
        
        # SEÑAL CALL: Alineación EMA alcista
        if indicators['ema_bullish_alignment']:
            signals.append("call")
            confidence += 30
            
            if indicators['stoch_k'] < 30:
                confidence += 15
            if indicators['rsi'] > 50:
                confidence += 10
        
        # SEÑAL PUT: Alineación EMA bajista
        if indicators['ema_bearish_alignment']:
            signals.append("put")
            confidence += 30
            
            if indicators['stoch_k'] > 70:
                confidence += 15
            if indicators['rsi'] < 50:
                confidence += 10
    ...
    def cci_dynamic_strategy(indicators):
        ...
        # SEÑAL CALL: CCI extremadamente oversold
        ...
        # SEÑAL PUT: CCI extremadamente overbought
...
🎯 Win Rate Esperado: {strategy_config['win_rate_expected']}%""")
            ...
            while self.running:
                try:
                    # Verificar conexión solo si IQ Option está disponible
                    if IQ_AVAILABLE:
                    ...
                    current_balance = 1000.0  # Default para testing
                    if IQ_AVAILABLE:
                        try:
...
🏁 Bot detenido automáticamente""")
                    ...
                    if IQ_AVAILABLE:
                        ...
                        candles = self._generate_mock_candles()
...
🛑 Bot detenido""")
    ...
    def _generate_mock_candles(self):
        """Genera datos simulados para testing cuando IQ Option no está disponible"""
        candles = []
        base_price = 1.0850
...
⏰ {datetime.datetime.now().strftime('%H:%M:%S')}""")
            ...
            # Simular resultado para testing
            if not IQ_AVAILABLE:
                # Simular operación con win rate basado en la estrategia
                win_rate = strategy_config['win_rate_expected']
                is_win = np.random.random() < (win_rate / 100)
                
                if is_win:
                    profit = amount * 0.8  # 80% payout típico
                    trade_result = "WIN"
                else:
                    profit = -amount
                    trade_result = "LOSS"
                ...
                time.sleep(5)  # Simular tiempo de trade
                
            else:
                # Abrir operación binaria real
                ...
                # Esperar resultado
                wait_time = strategy_config['expiry'] + 20
            ...
            # Actualizar balance simulado o real
            if IQ_AVAILABLE:
                new_balance = self.iq_api.get_balance()
            else:
                new_balance = 1000.0  # Balance simulado
...
🔥 {strategy_config['name']}""")
            ...
            return {
                "result": trade_result,
                "profit": profit,
                "order_id": "simulated" if not IQ_AVAILABLE else order_id,
                "amount": amount,
...
🔍 Indicadores Principales:
   ...
   • MACD: {'Bullish' if indicators['macd_bullish'] else 'Bearish'}
            ...
            with sessions_lock:
                ...
                for email, iq in list(user_sessions.items()):
                    try:
                        if IQ_AVAILABLE:
                            ...
                            # En modo simulación, mantener sesiones por tiempo
                            pass
                    except Exception as e:
                ...
                for email in emails_to_clean:
                    try:
                        ...
                        if IQ_AVAILABLE and email in user_sessions:
...
def health_check():
    ...
    try:
        iq_status = "available" if IQ_AVAILABLE else "simulation_mode"
...
def login():
    ...
    try:
        ...
        # Simular conexión exitosa si IQ Option no está disponible
        if not IQ_AVAILABLE:
            logger.info("📧 Modo simulación: creando sesión simulada")
            
            # Crear objeto simulado con métodos básicos
            class MockIQOption:
            ...
            iq = MockIQOption(email, password)
            
        else:
            # Función de conexión real con reintentos
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if hasattr(iq, 'api') and hasattr(iq.api, 'websocket_client'):
                            try:
                                iq.api.websocket_client.timeout = 25
                            except:
                                pass
                        ...
                        if connection_event.wait(timeout=25):
...
⏰ {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}""")
            ...
            return jsonify({
                ...
                "message": "Conexión exitosa (modo simulación)" if not IQ_AVAILABLE else "Conexión exitosa (datos limitados)"
            }), 200
    ...
    try:
        ...
        with bots_lock:
            if email in active_bots:
                ...
                if IQ_AVAILABLE and email in user_sessions:
                    try:
                        current_balance = user_sessions[email].get_balance()
                    except:
                        current_balance = 1000.0
...
if __name__ == '__main__':
    ...
    logger.info(f"🔧 IQ Option API: {'Disponible' if IQ_AVAILABLE else 'Modo Simulación'}")
    ...
    logger.info("   • ✅ Modo simulación disponible para testing")
    logger.info("=" * 80)
    
    if not IQ_AVAILABLE:
        logger.warning("⚠️ IQOptionAPI no está disponible. Ejecutando en modo simulación.")
        logger.info("💡 Para producción, instala: pip install git+https://github.com/Lu-Yi-Hsun/iqoptionapi.git")
    
    send_telegram_message(f"""🎯 *BOT OPCIONES BINARIAS PRO INICIADO*
⏰ {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
📍 Puerto: {port}
🔧 API: {'OK' if IQ_AVAILABLE else 'SIMULACIÓN'}
