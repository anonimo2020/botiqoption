...
class IQ_Option:
    ...
    def change_balance(self, Balance_MODE):
        def set_id(b_id):
            if b_id is None:
                raise ValueError(
                    f"No se encontró cuenta {Balance_MODE} en tu perfil de IQ Option. "
                    f"Verifica que tengas una cuenta {Balance_MODE} registrada y con fondos."
                )
            ...
            # Notificar al servidor de IQ Option via HTTP (cambia cuenta activa en el servidor)
            try:
                self.api.changebalance(b_id)
            except Exception as _e:
                logging.warning(f"HTTP changebalance call failed (non-critical): {_e}")
        ...
        if Balance_MODE == "REAL":
            ...
            logging.error(f"ERROR change_balance: modo desconocido '{Balance_MODE}'")
            raise ValueError(f"Modo de cuenta inválido: {Balance_MODE}")
